...
from .args import EVALUATION_MATCHES, EVALUATION_TEMPERATURE
from .misc import PrintColors as PC

def evaluate(
        rules: Rules,
        network: Network, 
        checkpoint_network: Network) -> tuple[int, int, int]:
    wins, losses, played = 0, 0, 0
    mcts1 = MCTS(rules, network)
    mcts2 = MCTS(rules, checkpoint_network)
    player = 0

    bar = tqdm(range(EVALUATION_MATCHES), desc="evaluating", bar_format="{l_bar}{bar}| game: {n_fmt}/{total_fmt} - {unit} - elapsed: {elapsed}")
    for _ in bar:
...
def play_match(rules: Rules, mcts1: MCTS, mcts2: MCTS) -> int:
    ...
    while winner is None:
        ...
        pi = mcts1.get_policy(observation, EVALUATION_TEMPERATURE) if cur_player == 1 else mcts2.get_policy(observation, EVALUATION_TEMPERATURE)
